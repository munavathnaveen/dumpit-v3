...
export class OrderTrackingService {
  ...
  static async getOrderDirections(orderData: OrderTrackingData): Promise<{
    ...
    try {
      if (!orderData.tracking?.currentLocation?.coordinates || 
          !orderData.shippingAddress?.location?.coordinates) {
  ...
  static formatETA(seconds: number): string {
    ...
    const isToday = now.getDate() === etaDate.getDate() && 
                    now.getMonth() === etaDate.getMonth() && 
                    now.getFullYear() === etaDate.getFullYear();
