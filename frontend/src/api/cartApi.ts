...
export const addToCart = async ({ productId, quantity }: { productId: string; quantity: number }): Promise<{ data: CartItem }> => {
    try {
        ...
        if (Array.isArray(response.data.data)) {
            ...
            const relevantItem = response.data.data.find((item) => item.product && (item.product._id === productId || (typeof item.product === "string" && item.product === productId)));
...
export const updateCartItem = async (itemId: string, quantity: number): Promise<{ data: CartItem }> => {
    try {
        ...
        if (Array.isArray(response.data.data)) {
            ...
            const relevantItem = response.data.data.find((item) => item.product && (item.product._id === itemId || (typeof item.product === "string" && item.product === itemId)));
