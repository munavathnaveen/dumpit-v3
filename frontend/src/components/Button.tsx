...
const Button: React.FC<ButtonProps> = ({ title, onPress, variant = "primary", type, disabled = false, loading = false, style, textStyle, icon, outline = false }) => {
    ...
    return (
        <TouchableOpacity activeOpacity={0.8} onPress={onPress} disabled={disabled || loading} style={[...getButtonStyles(), outline && styles.buttonOutline]}>
            {loading ? (
                <ActivityIndicator size="small" color={outline ? theme.colors.primary : theme.colors.white} />
            ) : (
                <View style={styles.buttonContent}>
                    {icon && <MaterialIcons name={icon as any} size={20} color={getIconColor()} style={styles.icon} />}
