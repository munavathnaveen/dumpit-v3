...
import { fetchNotifications, markNotificationRead, updateNotificationSettings } from "../store/userSlice";
...
const NotificationsScreen = () => {
    ...
    const filteredNotifications = isVendor
        ? notifications.filter(
              (notification) => notification.message.includes("order") || notification.message.includes("product") || notification.message.includes("payment") || notification.message.includes("shop")
          )
        : notifications;
    ...
    return (
        <View style={styles.container}>
            <ScreenHeader title="Notifications" showBackButton={true} onNotificationPress={() => navigation.navigate("Notifications")} />
            ...
            <View style={styles.statsContainer}>
                <Card3D style={styles.statsCard}>
                    <View style={styles.statsRow}>
                        ...
                        <View style={styles.statItem}>
                            <Text style={[styles.statValue, unreadCount > 0 ? styles.unreadValue : null]}>{unreadCount}</Text>
            ...
            <TouchableOpacity style={styles.settingsHeader} onPress={() => setExpanded(!expanded)}>
                <Text style={styles.settingsTitle}>Notification Settings</Text>
                <Feather name={expanded ? "chevron-up" : "chevron-down"} size={20} color={theme.colors.text} />
            </TouchableOpacity>
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity style={[styles.notificationItem, item.read && styles.notificationRead]} onPress={() => !item.read && handleMarkAsRead(item._id)}>
                        <Card3D elevation="small" style={styles.notificationCard}>
                            ...
                            {!item.read && (
                                <TouchableOpacity style={styles.markAsReadButton} onPress={() => handleMarkAsRead(item._id)}>
