...
const VendorAnalyticsScreen: React.FC = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            setError(null);
            const [analyticsData, statsData] = await Promise.all([fetchAnalytics(), getVendorOrderStats()]);
                    ...
                    <>
                        ...
                        <View style={styles.section}>
                            ...
                            <View style={styles.statsGrid}>
                                {renderStats("Total Revenue", `₹${analytics?.totalRevenue?.toLocaleString() || "0"}`, "cash-outline", theme.colors.success)}
                                {renderStats("Total Orders", analytics?.totalOrders?.toString() || "0", "receipt-outline", theme.colors.primary)}
                                {renderStats("Products", analytics?.totalProducts?.toString() || "0", "cube-outline", theme.colors.info)}
                                {renderStats("Pending Orders", analytics?.pendingOrders?.toString() || "0", "time-outline", theme.colors.warning)}
                            </View>
                        </View>
                        ...
                        {/* Export Data Button */}
                        <TouchableOpacity style={styles.exportButton} onPress={() => navigation.navigate("VendorImportExport")}>
