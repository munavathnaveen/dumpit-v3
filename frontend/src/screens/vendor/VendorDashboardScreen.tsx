...
const VendorDashboardScreen: React.FC = () => {
    ...
    const renderDashboardItem = (title: string, value: string | number, icon: string, color: string, onPress?: () => void) => (
        <TouchableOpacity style={styles.dashboardItem} onPress={onPress} disabled={!onPress}>
    ...
    if (loading) {
        return (
            <View style={styles.container}>
                <VendorLocationHeader title="Dashboard" showBackButton={false} onNotificationPress={handleNotificationPress} />
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={styles.errorContainer}>
                    ...
                    <TouchableOpacity style={styles.retryButton} onPress={() => navigation.replace("VendorDashboard")}>
        ...
        <View style={styles.container}>
            ...
            <ScrollView style={styles.scrollView} contentContainerStyle={styles.contentContainer}>
                ...
                <View style={styles.dashboardGrid}>
                    {renderDashboardItem("Total Revenue", analytics?.totalRevenue ? `₹${analytics.totalRevenue.toFixed(2)}` : "₹0.00", "cash-outline", theme.colors.success, () =>
                        navigation.navigate("VendorAnalytics")
                    )}

                    {renderDashboardItem("Orders", analytics?.totalOrders || 0, "receipt-outline", theme.colors.warning, () => navigation.navigate("VendorOrders"))}

                    {renderDashboardItem("Products", analytics?.totalProducts || 0, "cube-outline", theme.colors.primary, () => navigation.navigate("VendorProducts"))}

                    {renderDashboardItem("Pending Orders", analytics?.pendingOrders || 0, "time-outline", theme.colors.error, () => navigation.navigate("VendorOrders"))}
                </View>
                ...
                <Card3D style={styles.recentActivityCard} elevation="medium">
                    ...
                    {analytics?.recentOrders && analytics.recentOrders.length > 0 ? (
                        analytics.recentOrders.map((order, index) => (
                            <TouchableOpacity
                                key={order.id}
                                style={[styles.orderItem, index !== analytics.recentOrders.length - 1 && styles.orderItemBorder]}
                ...
                <View style={styles.actionButtonsContainer}>
                    <TouchableOpacity style={styles.actionButton} onPress={() => navigation.navigate("VendorAddProduct")}>
                    ...
                    <TouchableOpacity style={styles.actionButton} onPress={() => navigation.navigate("VendorShopSetup")}>
                        <Card3D style={styles.actionButtonCard} elevation="small">
                    ...
                    <TouchableOpacity style={styles.actionButton} onPress={() => navigation.navigate("VendorImportExport")}>
                        <Card3D style={styles.actionButtonCard} elevation="small">
                    ...
                    <TouchableOpacity style={styles.actionButton} onPress={() => navigation.navigate("VendorPayments")}>
                        <Card3D style={styles.actionButtonCard} elevation="small">
