...
import { View, Text, StyleSheet, TouchableOpacity, Image, ScrollView, TextInput, Alert, ActivityIndicator, Modal, Switch, Platform } from "react-native";
        ...
        <Card3D key={address._id} style={styles.addressCard}>
            <View style={styles.addressHeader}>
                ...
                <View style={styles.addressActions}>
                    <TouchableOpacity style={styles.addressEditButton} onPress={() => handleEditAddress(address)}>
                        <FontAwesome name="pencil" size={18} color={theme.colors.primary} />
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.addressDeleteButton} onPress={() => handleDeleteAddress(address._id)}>
                ...
                <Card3D style={styles.profileCard}>
                    ...
                    {isEditingProfile ? (
                        <View style={styles.editProfileForm}>
                            <Input label="Name" value={name} onChangeText={setName} placeholder="Your name" required />
                            <Input label="Phone" value={phone} onChangeText={setPhone} placeholder="Your phone number" keyboardType="phone-pad" required />
                            <View style={styles.buttonRow}>
                                ...
                                />
                                <Button title={isSubmittingProfile ? "Saving..." : "Save"} onPress={handleUpdateProfile} disabled={isSubmittingProfile} style={styles.formButton} />
                            </View>
                        </View>
                    ) : (
                        <Button title="Edit Profile" onPress={() => setIsEditingProfile(true)} icon="edit" style={styles.editButton} />
                    )}
                    ...
                    {isChangingPassword && (
                        <View style={styles.passwordForm}>
                            <Input label="Current Password" value={currentPassword} onChangeText={setCurrentPassword} placeholder="Your current password" secureTextEntry required />
                            <Input label="New Password" value={newPassword} onChangeText={setNewPassword} placeholder="New password" secureTextEntry required />
                            <Input label="Confirm New Password" value={confirmPassword} onChangeText={setConfirmPassword} placeholder="Confirm new password" secureTextEntry required />
                            <Button title={isSubmittingPassword ? "Updating..." : "Update Password"} onPress={handleUpdatePassword} disabled={isSubmittingPassword} />
                        </View>
                    )}
                </Card3D>
                ...
                <View style={styles.modalOverlay}>
                    <Card3D style={styles.modalContainer}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>{isEditingAddress ? "Edit Address" : "Add New Address"}</Text>
                            <TouchableOpacity
                                onPress={() => {
                                    setIsAddingAddress(false);
                                    resetAddressForm();
                        ...
                        <ScrollView style={styles.modalBody}>
                            <Input label="Address Name (e.g. Home, Work)" value={addressName} onChangeText={setAddressName} placeholder="Enter a name for this address" />
                            <Input label="Village/Town" value={village} onChangeText={setVillage} placeholder="Enter village or town" />
                            <Input label="Street" value={street} onChangeText={setStreet} placeholder="Enter street name" />
                            <Input label="District" value={district} onChangeText={setDistrict} placeholder="Enter district" />
                            <Input label="State" value={state} onChangeText={setState} placeholder="Enter state" />
                            <Input label="Pincode" value={pincode} onChangeText={setPincode} placeholder="Enter pincode" keyboardType="number-pad" maxLength={6} />
                            <Input label="Phone Number" value={addressPhone} onChangeText={setAddressPhone} placeholder="Enter phone number" keyboardType="phone-pad" maxLength={10} />
                            <View style={styles.switchContainer}>
                                <Text style={styles.switchLabel}>Set as default address</Text>
                                <Switch value={isDefault} onValueChange={setIsDefault} trackColor={{ false: theme.colors.lightGray, true: theme.colors.primary }} thumbColor={theme.colors.white} />
                            </View>
                            <Button title="Save Address" onPress={handleSaveAddress} style={styles.saveButton} variant="primary" />
                        </ScrollView>
                    </Card3D>
                </View>
            </Modal>
