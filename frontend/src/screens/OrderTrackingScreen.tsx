...
import { View, Text, StyleSheet, TouchableOpacity, ActivityIndicator, Dimensions, Modal, Image, Platform, Linking } from "react-native";
...
const OrderTrackingScreen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = OrderTrackingService.subscribeToOrderUpdates(orderId, (data) => {
            console.log("Received live tracking update:", data.tracking?.currentLocation ? `Position: ${data.tracking.currentLocation.coordinates.join(",")}` : "No location data");
    ...
    const loadTrackingData = async () => {
        try {
            ...
            console.log("Received tracking data:", data.tracking?.currentLocation ? `Position: ${data.tracking.currentLocation.coordinates.join(",")}` : "No location data");
            ...
            if (data.tracking?.currentLocation && data.shippingAddress?.location && !data.tracking.route) {
                try {
    ...
    const handleGetDirections = () => {
        ...
        if (url) {
            Linking.openURL(url).catch((err) => console.error("Error opening navigation app:", err));
        }
    };
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={styles.noTrackingContainer}>
                    <MaterialCommunityIcons name="map-marker-off" size={64} color={theme.colors.textLight} />
                    <Text style={styles.noTrackingText}>Tracking information is not available for this order yet</Text>
    ...
    return (
        <View style={styles.container}>
            ...
            {lastUpdateTime && <Text style={styles.lastUpdateText}>Last updated: {lastUpdateTime.toLocaleTimeString()}</Text>}

            {/* Main Map View */}
            <View style={styles.mapContainer}>
                <MapView ref={mapRef} provider={PROVIDER_GOOGLE} style={styles.map} region={initialRegion} showsUserLocation={true} showsCompass={true} showsScale={true}>
                    {trackingData.tracking.currentLocation && (
                        <Marker
                            coordinate={{
                                latitude: trackingData.tracking.currentLocation.coordinates[1],
                    ...
                    {routeCoordinates.length > 0 && <Polyline coordinates={routeCoordinates} strokeWidth={4} strokeColor={theme.colors.primary} />}
                </MapView>
            </View>
            ...
            <Card3D style={styles.trackingInfoCard}>
                <View style={styles.trackingHeader}>
                    <View style={styles.statusIconContainer}>
                        <FontAwesome
                            name={
                                trackingData.tracking.status === "preparing"
                                    ? "cube"
                                    : trackingData.tracking.status === "ready_for_pickup"
                                    ? "shopping-bag"
                                    : trackingData.tracking.status === "in_transit"
                                    ? "truck"
                    ...
                    <View style={styles.statusTextContainer}>
                        ...
                        <Text style={styles.statusText}>{trackingData.tracking.status ? OrderTrackingService.getStatusMessage(trackingData.tracking.status) : "Tracking your order..."}</Text>
                    </View>
                </View>
                ...
                <View style={styles.infoContainer}>
                    ...
                    {trackingData.tracking.lastUpdated && (
                        <View style={styles.infoItem}>
                            ...
                            <Text style={styles.infoText}>Last Updated: {new Date(trackingData.tracking.lastUpdated).toLocaleTimeString()}</Text>
                        </View>
                    )}
                </View>
                ...
                <View style={styles.buttonContainer}>
                    ...
                    <TouchableOpacity style={styles.deliveryInfoButton} onPress={() => setShowDeliveryInfo(true)}>
            ...
            {/* Delivery Info Modal */}
            <Modal visible={showDeliveryInfo} transparent={true} animationType="slide" onRequestClose={() => setShowDeliveryInfo(false)}>
                <View style={styles.modalContainer}>
                    <Card3D style={styles.modalContent}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>Delivery Information</Text>
                            <TouchableOpacity style={styles.closeButton} onPress={() => setShowDeliveryInfo(false)}>
                        ...
                        <View style={styles.deliveryInfoSection}>
                            ...
                            <Text style={styles.deliveryInfoText}>{trackingData.status.charAt(0).toUpperCase() + trackingData.status.slice(1)}</Text>
                        </View>

                        <View style={styles.deliveryInfoSection}>
                            <Text style={styles.deliveryInfoTitle}>Tracking Updates</Text>
                            <Text style={styles.deliveryInfoText}>You will receive real-time updates as your order progresses.</Text>
                        </View>

                        <TouchableOpacity style={styles.closeModalButton} onPress={() => setShowDeliveryInfo(false)}>
